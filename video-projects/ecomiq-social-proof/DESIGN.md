...
Delivered in two ratios from one timeline: **9:16 (1080×1920)** and **4:5 (1080×1350)**,
30fps, 38.5s. Safe area ~10% margins. (1:1 still generates on demand.)
...
- **message card** over the "what we do" lines — eyebrow → statement → down-arrow →
  payoff line → outlined pill. All of it white.
...
The **bottom of frame is left clear** for subtitles added later — ~30% in every ratio
(580px on 9:16, ~445px on 4:5, 300px on 1:1) — and the graphics band sits low enough that a
numeral never lands across a face. Note this is a *proportion*, not a fixed padding: on the
shorter 4:5 frame, centring the card already produces it, so 4:5 uses far less bottom
padding than 9:16 rather than more.
...
| Graphic accents only — down-arrow, baseline bar (never text) | `--brand-blue-tint` | `#9CD4FF` |
| **The one** hot accent — up-arrow, numeral bloom, CTA pill | `--brand-flame` | `#FF4C32` |
| **All text**, numerals included | `--brand-white` | `#FFFFFF` |

Flame orange is the only hot accent. No other accent colors.

**Every piece of text in the piece is white** — eyebrows, statements, qualifiers, the
end-card line, pill labels. Blue tint survives only on non-text graphics (the message
card's down-arrow, the graph card's baseline bar). Don't tint a word.
...
- **End card** (built to the client's reference): the lockup is the hero at 800px (9:16) /
  540px (1:1); beneath it one line at 60px / 42px, weight 700, **+0.16em tracking**,
  uppercase, in `--brand-blue-tint` — a tracked label, not a headline. Pill is 52px / 36px.
- **Hedvig Letters Serif is not used** in the current cut. The `@font-face` stays for future
  variants.
...
- Graphics **enter only** — the crossfade between beats is the exit. The end card is the
  one scene allowed an exit/hold; its tracked line arrives **word by word** (per-word
  inline-blocks on a 0.055s stagger). Never tween `letterSpacing` — it reflows text and
  snaps glyphs under the frame-seek engine, and lint rejects it.
...
- Don't give a full-bleed card `z-index` without `position` — z-index is inert on a static
  element, so the card drops into normal flow and jams against the top of frame. Every card
  is `position: absolute; inset: 0` with its hidden `opacity: 0` authored in CSS.
- Don't hand-edit the shorter-ratio compositions — they are generated by
  `scripts/make-ratios.py` from `index.html`. Re-run it after any master edit.
- Don't set text in `--brand-blue-tint`. All type is white; the tint is for graphic accents.
- Don't set display-size type below weight 800. Rethink Sans is a 400–800 variable font, so
  a 700 cut loads fine and lints clean — it just doesn't match, which reads as the wrong
  font rather than the wrong weight. Only the small tracked eyebrows go lighter (600–700).
