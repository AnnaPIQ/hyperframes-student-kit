// Plateau ad — v3. Continuous talking-head spine + VO. CUT between Sean talking
// (full-frame face) and FULL-SCREEN graphic / emphasis cards on key beats.
// Changes vs v2: emphasised hook card; polish pass (living card backgrounds,
// flame-gradient headlines, eyebrow accent, refined motion); proof split into a
// Driftsleep b-roll (orig 18–21s) then a +59% stat card; transitions are a clean
// blur/scale cross — no flame flash. Bottom ~1/6 left clear for captions. Voice-only.
...
// hook card
const HK_IN = 2.3, HK_MONTHS = 2.95, HK_TELL = 4.55, HK_OUT = 5.5;
// A: "more X" overload
const A_IN = 8.5, A1 = 8.86, A2 = 9.62, A3 = 10.95, A4 = 12.5, A_OUT = 13.3;
// B: system problem
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      * { margin: 0; padding: 0; box-sizing: border-box; }
      ...
      /* ---- top scrim + faint vignette over the face ---- */
      ...
      /* ---- logo chip (wrapped so the engine keeps it top-left) ---- */
      ...
      /* living background: soft drifting bloom + faint masked grid */
      ...
      /* card A — overload */
      #cardA .line { font-weight: 800; font-size: 104px; line-height: 1.06; letter-spacing: -.03em; color: #fff; opacity: 0; }
      #cardA .line .m { color: var(--brand-flame); }

      /* card B — system problem */
      #cardB .top { font-weight: 700; font-size: 46px; color: #b9c8dc; opacity: 0; margin-bottom: 20px; }
      #cardB .hero { font-weight: 800; font-size: 168px; line-height: .9; letter-spacing: -.04em; opacity: 0; }
      ...
      #cardG .l2 { font-weight: 800; font-size: 120px; line-height: .96; letter-spacing: -.04em; opacity: 0; margin-top: 22px; }
  ...
  <body>
    <div id="root" data-composition-id="${ID}" data-start="0" data-duration="${COMP}" data-width="1080" data-height="1350">
      ...
      <!-- CARD A : overload -->
      <div id="cardA" class="card clip" data-start="${A_IN - 0.4}" data-duration="${A_OUT - A_IN + 1}" data-track-index="7">
        <div class="cbloom"></div><div class="cgrid"></div>
      ...
      <!-- CARD B : system problem -->
      <div id="cardB" class="card clip" data-start="${B_IN - 0.4}" data-duration="${B_OUT - B_IN + 1}" data-track-index="8">
        <div class="cbloom"></div><div class="cgrid"></div>
        <div class="top">It's not an effort problem.</div>
        <div class="hero fg">A SYSTEM<br/>PROBLEM</div>
      </div>

      <!-- CARD C : friction checklist -->
      <div id="cardC" class="card clip" data-start="${C_IN - 0.4}" data-duration="${C_OUT - C_IN + 1}" data-track-index="9">
        <div class="cbloom"></div><div class="cgrid"></div>
      ...
      <div id="cardD" class="card clip" data-start="${D_IN - 0.4}" data-duration="${D_OUT - D_IN + 1}" data-track-index="10">
        <div class="cbloom"></div><div class="cgrid"></div>
        <div class="crow" id="d-sym"><span class="mk">&#10007;</span><span class="txt">The symptom</span></div>
        <div class="crow" id="d-cause"><span class="mk">&#10003;</span><span class="txt">The cause</span></div>
      </div>
    ...
    <script>
      ...
      // living background drift on every card (deterministic, evaluated per-frame)
      tl.to(".cbloom", { xPercent: 8, yPercent: -6, scale: 1.12, duration: 7, ease: "sine.inOut", repeat: ${Math.ceil(COMP / 7) + 1}, yoyo: true }, 0);

      // ---- clean blur/scale cross transitions (no flash) ----
      ...
      tl.fromTo("#proof video", { scale: 1.0 }, { scale: 1.08, duration: ${BROLL_OUT - BROLL_IN + 0.2}, ease: "none", transformOrigin: "50% 45%" }, ${BROLL_IN});
      tl.fromTo("#p-tag", { opacity: 0, y: -12 }, { opacity: 1, y: 0, duration: .4, ease: "power2.out" }, ${BROLL_IN} + .25);
      // cross into the stat card
