...
# 1. base track: founder 0-86.5 -> desk 86.5-93, Dryft overlay 57.5-65.5.
#    founder video + the muxed audio both come from founder-sync.mp4, so the
#    talking-head lips stay in sync in the gap windows. (The desk tail is fully
#    covered by the full-screen end card, so it needs no grading.)
ffmpeg -y -hide_banner -loglevel error -stats \
  -i assets/founder-sync.mp4 -i assets/founder-desk.mp4 -i assets/dryft-cut.mp4 \
  -filter_complex "\
    [0:v]trim=0:86.5,setpts=PTS-STARTPTS,fps=30[fv]; \
    [1:v]trim=0:6.5,setpts=PTS-STARTPTS,fps=30,scale=1080:1350:force_original_aspect_ratio=increase,crop=1080:1350[dv]; \
...
# 2. chroma-key graphics over base + mux the founder audio (same master = in sync)
ffmpeg -y -hide_banner -loglevel error -stats \
  -i renders/_base.mp4 -i "$KEY" -i assets/founder-sync.mp4 \
  -filter_complex "\
