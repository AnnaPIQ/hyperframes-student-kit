...
picture.** He is never on camera — montage throughout, then the brand end card, which
cross-dissolves in on the words *"want to see if we can help you."*
...
| Sub-line | `--brand-blue-tint`, with `--font-serif` italic on "more" |

The italic-serif emphasis word ("*more*") is the EcomIQ headline signature, kept small here
so the pill stays the loudest thing on the card.
...
```
      ...
      ( Tap the link ↓ )
        Find out more
```
The pill echoes Sean's closing line verbatim — he says *"Tap the link to find out more,"*
so the button takes the instruction and the sub-line completes the sentence.
...
rule wipes from centre → pill pops (`back.out(1.7)`) → sub-line lifts. The pill then
breathes and the arrow bobs so the 3.9 s hold doesn't go static.
